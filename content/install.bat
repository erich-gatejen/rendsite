...
IF NOT EXIST %1\install.sh  (
	echo Berzerq2 system not found at install_root.
	goto help
)
...
echo rendsite.jar.location = lib\rendsite.jar >> %1\etc\install.config.auto
echo rendsite.software.location = lib\rendsitesystem.jar >> %1\etc\install.config.auto
...
:help
	...
	echo   install_root = directory where rendsite is installed.
	echo   java_install = directory where the java jdk installed.
	echo EXAMPLE install.bat \opt\rendsite \opt\jdk1.6.0_03
